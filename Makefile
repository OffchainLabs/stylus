...
get_stylus_test_rust = $(wildcard $(stylus_test_dir)/$(1)/*.toml $(stylus_test_dir)/$(1)/src/*.rs $(stylus_test_dir)/$(1)/.cargo/*.toml) $(stylus_lang_rust)
...
$(stylus_test_keccak_wasm): $(stylus_test_keccak_src)
	cargo build --manifest-path $< --release --target wasm32-unknown-unknown --config $(stylus_test_dir)/keccak/.cargo/config.toml
