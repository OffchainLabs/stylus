...
func createL2BlockChain(
	...
	blockchain, err := arbnode.WriteOrTestBlockChain(chainDb, nil, nil, initReader, chainConfig, arbnode.ConfigDefaultL2Test(), 0)
	Require(t, err)
...
func Create2ndNodeWithConfig(
	...
	l2blockchain, err := arbnode.WriteOrTestBlockChain(l2chainDb, nil, nil, initReader, first.ArbInterface.BlockChain().Config(), arbnode.ConfigDefaultL2Test(), 0)
	Require(t, err)
