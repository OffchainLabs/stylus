...
use crate::{Bytes32, load_bytes32, store_bytes32, tx};

pub trait BackendStorage {
    fn new() -> Self;
    fn seed(&mut self) -> [u8; 32];
    fn load(&mut self, key: Bytes32) -> Bytes32;
    fn store(&mut self, key: Bytes32, value: Bytes32);
}
...
impl BackendStorage for MemoryBackendStorage {
    ...
    fn load(&mut self, key: Bytes32) -> Bytes32 {
        *self.slots.get(&key).unwrap_or(&Bytes32::default())
    }
...
pub struct CachingStorage<Backend: BackendStorage> {
    backend: Backend,
    save_on_drop: bool,
    cached_slots: HashMap<Bytes32, Bytes32, RandomState>,
    dirty_slots: HashSet<Bytes32, RandomState>,
    current_slot: usize,
    next_offset: usize,
}
...
impl<Backend: BackendStorage> CachingStorage<Backend> {
    ...
    pub fn get_slot(&mut self, key: Bytes32) -> Bytes32 {
        *self.cached_slots.entry(key).or_insert(self.backend.load(key))
    }

    pub fn set_slot(&mut self, key: Bytes32, data: Bytes32) {
