...
fn user_main(input: Vec<u8>) -> Result<Vec<u8>, Vec<u8>> {
    ...
    if data != B256::from(uint!(0_U256)) {
        // Transient data should have been initialized to zero
        // as new data
        return Err(input);
    }
    ...
    if data2 != B256::from(uint!(10_U256)) {
        // Transient data should have been set to 10 by transient_reenter
        // and not changed by transient_enter call
        return Err(input);
    }
